...
class PageForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not self.instance.pk and (not (page_type in 'staticpagecontent-static-menu', 'redirecttourlpagecontent-redirectstourl-menu')) and Page.objects.filter(page_type = page_type):
            self._errors["page_type"] = self.error_class([_(u'Stránka s tímto typem již existuje')])
        ...
        if (url.has_model()):
            if (not content):
                self._errors["content"] = self.error_class([_(u'Obsah stránky nesmí být prázdný')])
            elif (url.get_model_name() != '%s.%s' % (content.cast()._meta.app_label, content.cast()._meta.object_name.lower())):
                self._errors["content"] = self.error_class([_(u'Špatný typ obsahu stránky, prosím vyberte jiný')])
        else:
            if (content):
